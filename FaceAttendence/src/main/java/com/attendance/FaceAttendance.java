...
public class FaceAttendance {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // Load multiple datasets and merge their label maps
        Map<String, Integer> labelToIndex = new HashMap<>();
        // Example: load datasets from "dataset1" and "dataset2"
        loadDatasetLabels("dataset1", labelToIndex);
        loadDatasetLabels("dataset2", labelToIndex);
    ...
    private static void loadDatasetLabels(String datasetPath, Map<String, Integer> labelToIndex) {
        File baseDir = new File(datasetPath);
        if (!baseDir.exists() || !baseDir.isDirectory()) {
            System.err.println("Invalid dataset directory: " + datasetPath);
            return;
        }
        int classIndex = labelToIndex.size();
