...
public class FaceAttendanceApp extends JFrame {
        ...
        while (running) {
            ...
            for (Rect r : faces.toArray()) {
                ...
                if (!marked.containsKey(name)) {
                    ...
                    String timestamp = fmt.format(new java.util.Date());
                    log("Done " + name + " at " + timestamp);
                    markAttendance(name, timestamp);
                }
            }
    ...
    private void markAttendance(String name, String timestamp) {
        try (FileWriter fw = new FileWriter("attendance.csv", true);
             BufferedWriter bw = new BufferedWriter(fw);
             PrintWriter out = new PrintWriter(bw)) {
            out.println(name + "," + timestamp);
        } catch (IOException e) {
            log(" Error marking attendance: " + e.getMessage());
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        // Read actual user IDs from dataset folders
        File datasetDir = new File("dataset");
        if (!datasetDir.exists() || !datasetDir.isDirectory()) {
            throw new IllegalStateException("Dataset directory not found: " + datasetDir.getAbsolutePath());
        }

        // Create label mappings from folder names
        Map<String, Integer> l2i = new HashMap<>();
        Map<Integer, String> i2l = new HashMap<>();
        
        int classIndex = 0;
        for (File userFolder : Objects.requireNonNull(datasetDir.listFiles())) {
            if (userFolder.isDirectory()) {
                String userId = userFolder.getName();  // This will be the actual user ID/name
                l2i.put(userId, classIndex);
                i2l.put(classIndex, userId);
                classIndex++;
            }
        }
